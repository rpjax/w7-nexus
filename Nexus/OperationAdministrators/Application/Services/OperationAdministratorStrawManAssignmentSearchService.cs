...
public sealed class OperationAdministratorStrawManAssignmentSearchService
    : IOperationAdministratorStrawManAssignmentSearchService
{
    private const int SearchKeywordMaxLength = 200;

    private IAccountRepository _accounts { get; }

    public OperationAdministratorStrawManAssignmentSearchService(IAccountRepository accounts)
    {
        _accounts = accounts;
    }

    public async Task<IResult<SearchStrawMenToAssignResponse>> SearchStrawMenToAssignAsync(
        SearchStrawMenToAssignRequest request)
    {
        if (request is null)
            return RequestBodyRequiredResult<SearchStrawMenToAssignResponse>();

        var builder = Result.Create<SearchStrawMenToAssignResponse>();
        ...
        var query = _accounts.AsQueryable()
            .Where(a => a.Roles.Contains(Roles.StrawMan));
        ...
        var response = new SearchStrawMenToAssignResponse
        {
            Offset = offset,
            Limit = limit,
            Total = total,
