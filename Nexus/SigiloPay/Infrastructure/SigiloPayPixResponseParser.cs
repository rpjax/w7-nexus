...
{
    /// <summary>
    /// Plataforma whitelabel (mesmo modelo que Wintech): PK da transação = <c>transaction.id</c> no webhook;
    /// na criação PIX costuma vir como <c>transaction.id</c> aninhado ou <c>transactionId</c> na raiz.
    /// Persistido em <see cref="Nexus.Payments.Aggregates.Payment.GatewayTransactionId"/>.
    /// </summary>
    internal static SigiloPayPixPaymentResult ParseCreatePixResult(string json)
    {
        ...
        if (string.IsNullOrWhiteSpace(transactionId) || string.IsNullOrWhiteSpace(pixCode))
        {
            throw new InvalidOperationException(
                $"SigiloPay response did not include gateway transaction id (transaction.id or transactionId) and pix.code. Body: {json}");
        }
