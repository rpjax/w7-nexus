...
public sealed class BalanceSplitCalculationServiceTests
{
    ...
    private static BalanceSplitSnapshot ProfitShare(string accountId, decimal percentage, decimal amount) =>
        BalanceSplitSnapshot.Create(accountId, percentage, amount, SplitKind.ProfitShare).Value!;
    ...
    public async Task CalculateForCreditAsync_AppliesStrawManFeeAndDilutesProfitShare()
    {
        ...
        var splits = result.Value!.SplitSnapshot;
        ...
        var strawSplit = splits.Single(s => s.SplitKind == SplitKind.StrawManMovementFee);
    ...
    public async Task CalculateForCreditAsync_WhenFeeAlreadyApplied_DoesNotRecalculate()
    {
        ...
        var diluted = new[]
        {
            ...
            BalanceSplitSnapshot.Create("straw-d", 10m, 10m, SplitKind.StrawManMovementFee).Value!,
        };
        ...
        Assert.True(result.IsSuccess);
        Assert.Equal(2, result.Value!.SplitSnapshot.Count);
    ...
    public async Task CalculateForCreditAsync_DefaultFeeIsZero_KeepsOriginalSplits()
    {
        ...
        Assert.True(result.IsSuccess);
        Assert.Single(result.Value!.SplitSnapshot);
