...
public sealed class BankAccountAggregateTests
{
    ...
    public void DebitPartialBalance_DividesBalanceCorrectly()
    {
        ...
        var origin = BalanceOriginSnapshot.Create("op-1", "operator-1", "straw-1").Value!;
        var split = BalanceSplitSnapshot.Create("operator-1", 100m, 1000m, SplitKind.ProfitShare).Value!;
