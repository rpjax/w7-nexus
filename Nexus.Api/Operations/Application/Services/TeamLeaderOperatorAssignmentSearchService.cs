...
public sealed class TeamLeaderOperatorAssignmentSearchService : ITeamLeaderOperatorAssignmentSearchService
{
    ...
    public TeamLeaderOperatorAssignmentSearchService(
        ...
        SearchOperatorsToAssignRequest request)
    {
        ...
        if (scopeResult.IsFailure)
            return Result<SearchOperatorsToAssignResponse>.Failure(scopeResult.Errors);

        var operatorIds = TeamLeaderOperationScope.CollectOperatorIds(scopeResult.Value.OperationTeams).ToArray();

        if (operatorIds.Length == 0)
        {
            return builder
                .WithValue(new SearchOperatorsToAssignResponse
                {
                    Offset = offset,
                    Limit = limit,
                    Total = 0,
        ...
        var query = _accounts.AsQueryable()
            .Where(a => operatorIds.Contains(a.Id) && a.Roles.Contains(Roles.Operator));
        ...
        var response = new SearchOperatorsToAssignResponse
        {
            ...
            Total = total,
            Items = accounts
                .Select(a => a.ToTeamLeaderAccountDetails())
                .ToList()
        };
