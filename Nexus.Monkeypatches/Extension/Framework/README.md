...
| `bridge/` | Bridge v2 — protocolo event-driven MAIN ↔ ISOLATED ↔ SW ([docs](bridge/README.md)) |
| `bootstrap.js` | SW — monta relay isolated, busca runtime remoto e injeta no MAIN |
| `runtime/` | Bundle MAIN — `runtime.js`, `monkeypatch_manager.js`, `state.js`, `cache.js`, `config.js` |
...
5. MAIN posta `INVOCATION_REQUEST`; isolado encaminha ao SW (fire-and-forget); SW executa handler e empurra `INVOCATION_RESPONSE` via `tabs.sendMessage`
